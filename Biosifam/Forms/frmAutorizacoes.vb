Public Class frmAutorizações
    ...
    Dim idProcedimento As Integer, idProcedimento2 As Integer
    ...
    Private Sub CarregarAutorizacao()
        ...
        ElseIf ds.Tables(0).Rows(0).Item("tipo") = "A" Or ds.Tables(0).Rows(0).Item("tipo") = "H" Then    ' ambulatorial/hospitalar - NAO usar tabela de autorizacao
        ...
        ElseIf ds.Tables(0).Rows(0).Item("tipo") = "E" Then    ' exames
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles btnAutorizar.Click
        ...
        If _u.vbUsuarioSuporte Then
            If MsgBox("Usuário Suporte deseja prosseguir mesmo assim ?", vbYesNo) = vbNo Then Exit Sub
        Else
            MsgBox("Rotina em implentação, aguarde próximas versões.")
            Exit Sub
        End If
        ...
        ElseIf TabPage2.Visible Then        'exames

            ' veirica existencia de cid10 para o mesmo paciente e outras matriculas

            _PG.Conectar()
            drGeneric = _PG.DrQuery("SELECT id_atendimento_autorizacao from atendimento_autorizacao WHERE (tipo='A'or tipo='H') and id_pessoa=" & CType(cmbPaciente.SelectedItem, ComboData).Id & " and codigo_procedimento='" & CmbCBHPM.SelectedItem.ToString.Substring(0, 8) & "' and situacao='P'")
            If drGeneric.HasRows Then
                MsgBox("Já existe uma autorização em aberto para o CBHPM informado.", MsgBoxStyle.Information, "Impossível gravar autorização")
                GoTo Encerrar
            End If
            drGeneric = _PG.DrQuery("SELECT id_atendimento_autorizacao from atendimento_autorizacao aa, pessoas p1, pessoas p2 WHERE (tipo='A'or tipo='H') and aa.id_pessoa=" & CType(cmbPaciente.SelectedItem, ComboData).Id & " and p1.cpf=p2.cpf and codigo_procedimento='" & CmbCBHPM.SelectedItem.ToString.Substring(0, 8) & "' and aa.situacao='P'")
            If drGeneric.HasRows Then
                MsgBox("Já existe uma autorização em aberto para o CPF e CBHPM informados.", MsgBoxStyle.Information, "Impossível gravar autorização")
                GoTo Encerrar
            End If
            Dim Strtipo As String = "H"   ' hospitalar
            If vbAutorizacaoExiste = False Then
                If Val(txtPrazoInternacao.Text) = 0 Then Strtipo = "A"
                If idProcedimento2 > 2 Then idProcedimento &= ", " & idProcedimento2
                _PG.Execute("begin") ' =============================================================================================================================================
                ' prestador aqui é o local que fará o atendimento ambulatorio ou hospital
                ' medico é o solicitante, tipo documento = B = biosifam, documento=id mediico colicitante
                vsSql = "INSERT INTO atendimento_autorizacao ( matricula, id_prestador, Data, situacao, tipo, id_pessoa, sessoes_autorizadas, sessoes_realizadas, solicitante, solicitante_tipo_doc, solicitante_documento, codigo_procedimento, observacao, autorizacao_medica, prazos, ip, dt_alteracao, login)"
                vsSql &= " values('" & txtMatricula.Text & "' , " & CType(cmbHospital.SelectedItem, ComboData).Id & ",current_date,'P','" & Strtipo & "'," &
                    CType(cmbPaciente.SelectedItem, ComboData).Id & ",0,0,'" & oMedicoConveniado.Nome & "','B','" & oMedicoConveniado.Id & "','" & idProcedimento & "','" &
                    txtObservacaoSolicitacao.Text & "','$imagem_nome'," & Val(txtPrazoInternacao.Text) & ",'" & _t.ObtemEnderecoIP & "',current_timestamp,'" & frmLogin.TxtUsuario.Text & "')"
                If _PG.Execute(vsSql) Then
                    drGeneric = _PG.DrQuery("select lastval()")
                    If drGeneric.HasRows Then
                        drGeneric.Read()
            ...
            Else
                If strSituacao = "P" Then MsgBox("Autorização está aguardando autorização !") : Exit Sub
                If oMedicoConveniado.Categoria = "M" Then MsgBox("Médicos não podem finalizar Procedimentos Ambulatoriais/Hospitalares !") : Exit Sub
...
Encerrar:
    ...
    Private Sub cmbTipo_SelectedIndexChanged(sender As Object, e As EventArgs)
        ...
        If Val(txtPrazoInternacao.Text) = 0 Then
    ...
    Private Sub chkSemInternacao_CheckedChanged(sender As Object, e As EventArgs) Handles chkSemInternacao.CheckedChanged
        txtPrazoInternacao.Text = "0"
    End Sub

    Private Sub CmbCBHPM_SelectedIndexChanged(sender As Object, e As EventArgs) Handles CmbCBHPM.SelectedIndexChanged
